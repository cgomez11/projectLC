...
def maxPastMass(tree, index, partType='stars'):
    ...
    corr_snapshots = tree['SnapNum'][index: index + branchSize]
    pos_max_mass = np.argmax(masses)
    return np.max(masses), corr_snapshots[pos_max_mass]
...
def numMergers(tree, minMassRatio=1e-10, massPartType='stars', index=0):
    ...
    while fpID != -1:
        ...
        fpMass, fpSnap  = maxPastMass(tree, fpIndex, massPartType)
        ...
        while npID != -1:
            ...
            if fpMass > 0.0 and npMass > 0.0:
                ...
                if ratio >= minMassRatio and ratio <= invMassRatio:
                    numMergers += 1
                    snaps_mergers.append(fpSnap)
                    #print('first progenitor index:', fpIndex)
                    #print('fp max mass snap:', fpSnap)
                    #print('Next progenitor index:', npIndex)
                    #print('np max mass snap:', npSnap)

            npID = tree['NextProgenitorID'][npIndex]

        fpID = tree['FirstProgenitorID'][fpIndex]

    return numMergers, snaps_mergers
